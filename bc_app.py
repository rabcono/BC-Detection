...
navbar = dbc.NavbarSimple(
    brand='Interest Rate Predictor',
    brand_href='/',
    children=[
        dbc.NavItem(dcc.Link('Predictions', href='/predictions', className='nav-link')),
        # dbc.NavItem(dcc.Link('Process', href='/process', className='nav-link')),
        dbc.NavItem(dcc.Link('About', href='/about', className='nav-link')),
        dbc.NavItem(dcc.Link('Jupyter Notebook', href="https://github.com/rabcono/BC-Detection/blob/main/SVM%20Breast%20Cancer.ipynb", className='nav-link', target="_blank")),
...
footer = dbc.Container(
    dbc.Row(
        dbc.Col(
            html.P(
                [
                    html.Span('Robert Allegra', className='mr-2'),
                    ],
                className='lead'
            )
        )
